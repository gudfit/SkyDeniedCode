...
def main():
    ...
    train_df = pd.read_csv('processedDataTest/train_set.csv')
    val_df = pd.read_csv('processedDataTest/validation_set.csv')
    test_df = pd.read_csv('processedDataTest/test_set.csv')
    ...
    if not os.path.exists('adapted_data'):
        os.makedirs('adapted_data')
    
    # Save to pickle files for easy loading in training script
    import pickle
    
    with open('adapted_data/train_data.pkl', 'wb') as f:
        pickle.dump((train_chains, train_labels), f)
    
    with open('adapted_data/val_data.pkl', 'wb') as f:
        pickle.dump((val_chains, val_labels), f)
    
    with open('adapted_data/test_data.pkl', 'wb') as f:
