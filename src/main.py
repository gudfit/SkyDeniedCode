...
# --- Configuration --- (Moved near top for clarity)
OUTPUT_DIR = '.' # Save models/plots in the current directory or specify path
...
# Define features (Ensure these columns exist in your source data before pickling!)
# Flight_Delay is the target, should not be in input features for the model preprocessor
CATEGORICAL_FEATURES = ['Carrier_Airline', 'Origin', 'Dest', 'Orientation'] # Removed Tail_Number for simplicity if not needed as direct feature
NUMERICAL_FEATURES = ['Flight_Duration_Minutes', 'FTD', 'PFD'] # Removed Flight_Delay
TEMPORAL_FEATURES = ['Schedule_DateTime']

# Model Hyperparameters
# CHAIN_LENGTH will be determined from loaded data
LSTM_HIDDEN_SIZE = 128
CNN_CHANNELS = [32, 64]
# --- USE 2D KERNELS for Conv2d --- Example: [(1,1), (3,3)]
...
# Custom Dataset (kept simple for compatibility with pickled data, but less efficient)
class SimpleFlightChainDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Ensure chain has expected length (padding might be needed if chains vary)
        current_chain_len = len(chain)
        if current_chain_len != self.chain_length:
            # Handle length mismatch if necessary (e.g., skip, pad chain data)
            # This example assumes chains are pre-padded or filtered to be uniform length
            # If not, more complex handling is needed here or in data prep.
            print(f"Warning: Chain {idx} has length {current_chain_len}, expected {self.chain_length}. Skipping or Padding needed.")
            # For now, let's create a zero tensor and hope transform_single_flight works
            # This might still cause issues downstream if model expects fixed T
            pass # Or return dummy data / raise error


        # Create a zero tensor of the right size [T, D]
        # Use self.chain_length for the time dimension
        ...
        if not (0 <= label < 5): # Assuming 5 classes (0-4)
             print(f"Warning: Invalid label {label} at index {idx}. Setting to 0.")
             label = 0


        return torch.FloatTensor(X), torch.LongTensor([label]) # Ensure label is list/tensor
...
def main():
    ...
    try:
        ...
        # Add 'src' directory to Python path if necessary
        if 'src' not in sys.path:
             sys.path.insert(0, 'src')

        try:
            # Import components from your model file (make sure it exists in src/)
        ...
        preprocessor = FlightDataPreprocessor(
            ...
            numerical_features=NUMERICAL_FEATURES, # Use corrected features
            temporal_features=TEMPORAL_FEATURES
            # NO embedding_dims argument here!
        )
        ...
        # --- Defensive Column Handling (Optional but recommended before fit) ---
        ...
        if TEMPORAL_FEATURES[0] not in train_df_for_fitting.columns: # Assuming only 'Schedule_DateTime'
        ...
        # --- End Defensive Column Handling ---


        # Fit the preprocessor
        preprocessor.fit(train_df_for_fitting)
        # Preprocessor should now have calculated self.total_dim correctly if using the full version
        ...
        if os.name == 'nt': num_workers = 0 # Often helps on Windows
        ...
        # --- Initialize Model ---
        num_classes = 5 # Delay levels (0-4) - ensure this matches your labels
        ...
        # Load the best model explicitly (train_model already loads it, but good practice)
        try:
             ...
             class_names = ['On Time/Early', 'Slight Delay', 'Moderate Delay', 'Significant Delay', 'Severe Delay'] # Use descriptive names
        ...
        try:
            ...
            preprocessor.save(PREPROCESSOR_SAVE_PATH) # Use preprocessor's save method if available (recommended)
            # Fallback to pickle if save method doesn't exist
